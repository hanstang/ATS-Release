...
class operationTask(threading.Thread):
	...
	def __init__(self, listCmd, serial, yawcamURL, captureDir):
	...
	def btnRun(self):
		...
		if self.ctr_command > 1:
			if self.urlValue.get() != "" and self.dirValue.get() != "" :
				...
				self.RUN_UI()
				#Run Operation
				self.operationTask=operationTask(self.command_list,self.ser,self.urlValue,self.dirValue)
...
window = Tkinter.Tk()
window.title("Tranzas STB Automation Test Ver 0.2")
